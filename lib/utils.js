...
const checkResult = async (res) => new Promise((resolve,reject) => {
  console.log('checkResult');
  console.log(res.json);
  if (Object.prototype.hasOwnProperty.call(res.data, 'err')) {
    console.log('avem o problema');
    return reject(new Error('No first name'));
  }
  return resolve();
});
...
const validatePassword = async (password) => {
  const passwordRegex = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[ #?!@$%^&*-]).{1,}$/;
